...
const Bottle = ({name,price}) => {
    // js
    function handleBuy() {
        alert('Please wait while will redirect your to the checkout page')
    }
    function textBuyworking() {
        alert('Please wait while will textBuyworking, and redirect your to the checkout page')
    }
    let handleSave = ()=> {
  ...
  return (
    <div id='Bottle' >
        {/* jsx */}
        <h2 style={{color: 'red',backgroundColor: 'grey'}} >I am {name?name: 'Eva'} Bottle. You can get me for just ${price?price: 1} </h2>
        <p>Lorem ipsum dolor sit amet.</p>
        <button onClick={()=> handleBuy()} >Buy Now</button>
        <button onClick={()=> textBuyworking()} >Text now</button>
        <button onClick={handleSave} >Save for Latter</button>
        <h4 className={global.h4} >This is Heading 3</h4>
        <p className={global.h3}  >Lorem ipsum dolor sit amet consectetur adipisicing elit. Tempore nostrum eius dolorem rem minus magnam voluptate porro laudantium dolorum laboriosam?</p>
        {/* tailwind practice */}
